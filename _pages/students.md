...
{% for member in site.data.students.grad_curr %}
...
## Alumni
{% assign number_printed = 0 %}
{% for member in site.data.students.grad_alum %}
...
<ul>
  {% for student in site.data.students.undergrad_curr %}
    <li>
      <strong>{{ student.name }}</strong> ({{ student.year }} year), <em>{{ student.subject }}</em><br />

      {% if student.fund or student.pubs %}
          {% if student.fund %}
            {% for award in student.fund %}
      🎖 {{ award }}<br />
            {% endfor %}
          {% endif %}

          {% if student.pubs %}
            {% for paper in student.pubs %}
      📄 {{ paper }}<br />
            {% endfor %}
          {% endif %}
